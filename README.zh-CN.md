...
- **插件系统** — 生命周期钩子，支持自定义处理
- **多站点** — 单一配置管理多个网站
- **动态站点管理** — 运行时通过 REST API 添加、删除、扫描站点
- **Docker** — 容器化部署
...
| `POST /api/sites` | 动态添加新站点 |
| `DELETE /api/sites/:slug` | 删除站点 |
| `POST /api/sites/:slug/scan` | 触发指定站点扫描 |
| `POST /api/rescan` | 重新扫描所有站点，或传入 `{ "url": "..." }` 扫描任意地址 |

### 站点管理 API

通过 API 动态管理站点 —— 无需 SSH 进服务器手动改配置文件：

```bash
# 添加新站点
curl -X POST http://localhost:3141/api/sites \
  -H 'Content-Type: application/json' \
  -d '{"url": "https://example.com", "name": "Example", "description": "我的站点"}'
...
修改会自动持久化到 `agentsite.config.yaml`。
